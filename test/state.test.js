...
describe('State', () => {
  ...
  it('should use state as generated by reducers', () => {
    const Jumpstate = require('../src/index')
    const State = Jumpstate.State
    const CreateJumpstateMiddleware = Jumpstate.CreateJumpstateMiddleware
    const Redux = require('redux')
    ...
    const Reducer = State({
      ...
      setAnotherValue (state, payload) {
        return { anotherValue: payload }
      }
    })
    ...
    Reducer.setValue(100)
    Reducer.setAnotherValue('foobar')

    expect(Store.getState().anotherValue).toEqual('foobar')
